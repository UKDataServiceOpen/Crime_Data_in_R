...
There are a couple of ways to introduce external data. 

```{r external data}
# Use `st_read()` to load most external data because it supports a variety of common data sources. 
# df <- sf::st_read("data.shp", stringsAsFactors = FALSE)

# `sf` has a function to work with `sp` objects 
# sf_object <- sf::st_as_sf(sp_object)

# You can use [`tidycensus`](https://walkerke.github.io/tidycensus/) or [`tigris`](https://github.com/walkerke/tigris) packages to access spatial data as `sf` objects. 
# `tidycensus`, the `geometry = TRUE` option should be used.
# stl_race <- tidycensus::get_acs(geography = "tract", state = 29, county = 510, year = 2017, table = "B02001", output = "wide", geometry = TRUE)

# In `tigris`, the `class = "sf"` option should be used.
# stl_tracts <- tigris::tracts(state = 29, county = 510, class = "sf")

# Tabular data must be merged with a shapefile or spatial data set that contains the corresponding geometry. A common scenario might be census geography downloaded using [`tigris`](https://github.com/walkerke/tigris) and then combined using [`dplyr`](https://dplyr.tidyverse.org) according to an identification number column such as `GEOID`. 
